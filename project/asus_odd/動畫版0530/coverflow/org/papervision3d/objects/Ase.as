...
class org.papervision3d.objects.Ase extends Mesh
{
	...
	*/
	private static var INTERNAL_SCALING:Number = 50;
	...
	*/
	private function loadAse(filename:String)
	{
		...
		{
			if (!sucess) {
				Papervision3D.log(filename + ": ASE File Error");
				return;
			}
			...
			while (lines.length)
			{
				...
				case 'MESH_TFACELIST' :
					...
					while ((content = String(lines.shift())).indexOf('}') < 0)
					{
						...
						// separate here
						faces[num].uv = [uvs[parseInt(mtfl[1])], uvs[parseInt(mtfl[2])], uvs[parseInt(mtfl[3])]];
						num++;
					}
					break;
				}
